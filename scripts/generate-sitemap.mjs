...
const localizedKeyboardLocales = ["es", "de", "fr", "it", "pt", "pl", "tr", "uk", "id", "zh", "ja", "ko"];
...
function routeFromFile(filePath) {
  ...
  const routePath = withoutExtension.endsWith("/index") ? withoutExtension.slice(0, -"index".length) : withoutExtension === "index" ? "" : `${withoutExtension}/`;
...
const generatedKeyboardRoutes = localizedKeyboardLocales.map((locale) => `/${locale}/keyboard-test/`);
const routes = [...new Set([...pageFiles.map(routeFromFile).filter(Boolean), ...generatedKeyboardRoutes])].sort((a, b) => a === "/" ? -1 : b === "/" ? 1 : a.localeCompare(b));
