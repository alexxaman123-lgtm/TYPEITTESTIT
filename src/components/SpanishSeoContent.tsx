export default function SpanishSeoContent() {
  return (
    <section id="typing-practice" className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="font-body text-text-muted space-y-12 [&_h2]:font-heading-4 [&_h2]:text-ink [&_h3]:font-heading-5 [&_h3]:text-ink [&_h3]:mt-8 [&_p]:mt-3 [&_ul]:mt-4 [&_ul]:list-disc [&_ul]:pl-6 [&_ul]:space-y-2 [&_strong]:text-ink [&_strong]:font-semibold">
        ...
        <div>
          ...
          <p>Entre las consultas relacionadas aparecen <strong>test de mecanografía en español</strong>, <strong>test de mecanografía en línea</strong>, <strong>test de mecanografía online gratis</strong>, <strong>test de mecanografía en 1 minuto</strong>, <strong>test de mecanografía palabras por minuto</strong>, <strong>prueba de velocidad de escritura</strong> y <strong>prueba de velocidad de escritura en computadora</strong>. Esta página reúne esas necesidades dentro de una misma herramienta, sin crear páginas duplicadas para cada variación.</p>
        </div>
        <div>
          <h2>Variaciones y tipos de test</h2>
          <p>También puedes encontrar búsquedas como <strong>test de mecanografia</strong> (sin tilde), <strong>test de mecanografía sin acentos</strong>, <strong>test de mecanografía sin tildes</strong>, <strong>test de mecanografía sin tilde</strong>, <strong>test de mecanografía sin acentos ni mayúsculas</strong>, <strong>test de mecanografía español sin acentos</strong>, <strong>test de mecanografía velocidad</strong>, <strong>test de mecanografía teclado numérico</strong>, <strong>test de mecanografía numérica</strong> y <strong>test de mecanografía Dvorak</strong>. El objetivo de estas consultas suele ser comparar velocidad, practicar un formato concreto o trabajar con un tipo de teclado determinado.</p>
          <p>Otras variantes visibles en búsquedas incluyen <strong>test de tipeo</strong>, <strong>prueba de mecanografía</strong>, <strong>mecanografía test</strong>, <strong>mecanografía online gratis</strong>, <strong>typing test español</strong>, <strong>test de mecanografía fast fingers</strong>, <strong>prueba de velocidad de escritura teclado</strong>, <strong>prueba de velocidad de escritura sin acentos</strong>, <strong>prueba de velocidad de escritura en el teclado</strong> y <strong>prueba de velocidad de escritura en teclado</strong>. FreeTypingTestGoat se centra en ofrecer una prueba directa y una práctica medible desde el navegador.</p>
        </div>
        <div>
        ...
        <div>
          <h2>Tabla orientativa de velocidad de escritura</h2>
          <p>La velocidad puede variar según la dificultad del texto, la duración, la familiaridad con las palabras y la concentración. Utiliza estos rangos como una referencia práctica, no como una calificación estricta.</p>
          <div className="mt-6 overflow-hidden rounded-[24px] border border-hairline bg-canvas shadow-sm">
            <div className="grid grid-cols-3 border-b border-hairline bg-canvas-soft px-4 py-3 font-caption text-text-muted sm:grid-cols-4"><span>Nivel</span><span>WPM</span><span className="col-span-1 hidden sm:block">Qué significa</span><span>Siguiente objetivo</span></div>
            {[["En desarrollo","0–29","Construye control básico del teclado, ritmo y precisión.","30"],["Cotidiana","30–44","Velocidad práctica para tareas habituales y estudio.","45"],["Buena","45–59","Ritmo sólido para escritura y tareas de oficina.","60"],["Rápida","60–79","Velocidad práctica alta para muchas tareas de escritura.","80"],["Avanzada","80–99","Alta velocidad con atención a mantener la precisión.","100"],["100+","100+","Un nivel muy rápido alcanzado por mecanógrafos con mucha práctica.","—"]].map(([level, range, meaning, next]) => (
              <div key={level} className="grid grid-cols-3 items-center gap-3 border-b border-hairline px-4 py-4 last:border-b-0 sm:grid-cols-4"><span className="font-heading-6 text-ink">{level}</span><span className="font-mono font-bold text-accent">{range}</span><span className="col-span-1 hidden font-body-sm text-text-muted sm:block">{meaning}</span><span className="text-right font-body-sm font-semibold text-text-muted">{next === "—" ? "—" : `↑ ${next} WPM`}</span></div>
            ))}
          </div>
        </div>
      </div>
