  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        if (authError || !user) { setSignedIn(false); setUsername(null); setEmail(null); setMemberSince(null); setEntries([]); setLoading(false); return; }
        setSignedIn(true);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
      if (!mounted) return;
      if (event === "SIGNED_OUT") { setSignedIn(false); setUsername(null); setEmail(null); setMemberSince(null); setEntries([]); setError(null); setLoading(false); return; }
        ...
        {!loading && !signedIn ? (
          ...
          <>
            <div className="flex flex-col gap-4 border-b border-hairline pb-7 sm:flex-row sm:items-end sm:justify-between">
              <div><p className="font-label uppercase tracking-[0.2em] text-text-muted">{t.account}</p><h1 className="mt-2 font-heading-2 text-ink">{t.typingHistory}</h1><p className="mt-2 font-body text-text-muted">{username || t.yourPersonalProgress}</p></div>
              <button type="button" onClick={() => window.location.reload()} className="rounded-full border border-hairline px-4 py-2 font-link text-ink hover:bg-canvas-soft">{t.refresh}</button>
            </div>
            {error ? (
              <div className="rounded-2xl border border-hairline bg-canvas-soft px-5 py-10 text-center"><p className="font-body text-text-muted">{error}</p><button type="button" onClick={() => window.location.reload()} className="mt-5 rounded-full border border-hairline px-4 py-2 font-link text-ink hover:bg-canvas">{t.tryAgain}</button></div>
            ) : (
              <>
                <div className="mt-7 grid gap-4 sm:grid-cols-2"><MetricCard label={t.bestWpmLabel} value={bestWpm ? bestWpm.wpm.toFixed(1) : "\u2014"} suffix=" WPM" /><MetricCard label={t.bestAccuracyLabel} value={bestAccuracy ? bestAccuracy.accuracy.toFixed(2) : "\u2014"} suffix="%" /></div>
                <AccountInfoCard username={username} email={email} memberSince={memberSince} locale={locale} t={t} />
...
function AccountInfoCard({ username, email, memberSince, locale, t }: { username: string | null; email: string | null; memberSince: string | null; locale: Locale; t: AccountText }) {
  ...
  return (
    <div className="mt-7 rounded-2xl border border-hairline bg-canvas-soft p-5">
      <p className="font-label uppercase tracking-[0.14em] text-text-muted">{t.accountInfoHeading}</p>
      <div className="mt-3 grid gap-3 sm:grid-cols-3">
        <div><p className="font-label uppercase tracking-[0.12em] text-text-faint">{t.usernameLabel}</p><p className="mt-1 truncate font-body-sm text-ink">{username || t.notProvided}</p></div>
        <div><p className="font-label uppercase tracking-[0.12em] text-text-faint">{t.emailLabel}</p><p className="mt-1 truncate font-body-sm text-ink">{email || t.notProvided}</p></div>
        <div><p className="font-label uppercase tracking-[0.12em] text-text-faint">{t.memberSinceLabel}</p><p className="mt-1 truncate font-body-sm text-ink">{memberSinceLabel}</p></div>
      </div>
    </div>
  );
}
