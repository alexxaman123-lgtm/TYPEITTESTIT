  ...
  return (
    <section id="how-it-works" className="mx-auto max-w-5xl scroll-mt-20 px-4 py-16 sm:px-6 lg:px-8">
      <div className="text-center">
        <BlurTypeReveal as="h2" className="font-heading-4 text-ink" text="Cómo funciona" by="char" stagger={34} />
        <BlurTypeReveal
          as="p"
          className="mx-auto mt-3 max-w-2xl font-body text-text-muted"
          text="Test de mecanografía Cabra está diseñado para que entiendas la prueba en segundos: elige una dificultad, escribe, mide tu rendimiento y vuelve a practicar."
          by="word"
          stagger={22}
          delay={220}
          caret={false}
      ...
      <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4" data-reveal-group="alternate">
        {STEPS.map((step, i) => (
          <div key={step.title} className="premium-card rounded-[24px] border border-hairline bg-canvas p-6 shadow-sm">
